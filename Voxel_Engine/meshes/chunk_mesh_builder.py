...
def pack_data(x, y, z, voxel_id, face_id, ao_id, flip_id):
    # x: 6bit  y: 6bit  z: 6bit  voxel_id: 8bit  face_id: 3bit  ao_id: 2bit
    # flip_id: 1bit
...
def is_void(local_voxel_pos, world_voxel_pos, world_voxels):
    ...
    voxel_index = (
        x % GENERATION_INTENSITY
        + z % GENERATION_INTENSITY * GENERATION_INTENSITY
        + y % GENERATION_INTENSITY * CHUNK_AREA
    )
    ...
    for x in range(GENERATION_INTENSITY):
        for y in range(GENERATION_INTENSITY):
            for z in range(GENERATION_INTENSITY):
                voxel_id = chunk_voxels[
                    x + GENERATION_INTENSITY * z + CHUNK_AREA * y
                ]
                ...
                if is_void((x, y + 1, z), (wx, wy + 1, wz), world_voxels):
                    ...
                    v2 = pack_data(
                        x + 1, y + 1, z + 1, voxel_id, 0, ao[2], flip_id
                    )
                ...
                if is_void((x + 1, y, z), (wx + 1, wy, wz), world_voxels):
                    ...
                    v2 = pack_data(
                        x + 1, y + 1, z + 1, voxel_id, 2, ao[2], flip_id
                    )
                ...
                if is_void((x, y, z + 1), (wx, wy, wz + 1), world_voxels):
                    ...
                    v2 = pack_data(
                        x + 1, y + 1, z + 1, voxel_id, 5, ao[2], flip_id
                    )
