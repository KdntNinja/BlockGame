...
class Menu:
    ...
    def base_menu(self):
        ...
        self.menu.add.text_input("Name :", default=self.config.get("name", self.default_config.name), onchange=self.change_config("name", self.temp_config["name"]))
    ...
    def create_settings_menu(self):
        ...
        settings_menu.add.selector("Fullscreen :", [("OFF", 0), ("ON", 1)],
                                   onchange=lambda value, index: self.toggle_fullscreen(value), default=self.default_config.fullscreen)
        settings_menu.add.selector("Mode :", [("Spectator", 0), ("Survival", 1)],
                                   onchange=lambda value, index: self.change_config("mode", value), default=self.default_config.mode)
        settings_menu.add.selector("World :", [("Initial", 0), ("Constant", 1)],
                                   onchange=lambda value, index: self.change_config("world", value), default=self.default_config.world)
